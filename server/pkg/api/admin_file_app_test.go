...
func TestInitializeFileAppHandler(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"already_ready", `UPDATE usage SET file_app_ready = TRUE`,
		...
		{"ineligible", `INSERT INTO collections(owner_id, encrypted_key, key_decryption_nonce, name, type, attributes, updation_time, app)
			VALUES (1, 'key', 'nonce', 'Photos', 'album', '{}', 1, 'photos'),
				(1, 'key', 'nonce', 'Locker', 'album', '{}', 1, 'locker');
			...
			http.StatusOK, `{"initialized":false,"reason":"file app provenance is ineligible: Locker candidate has cross-app or unsupported-app history"}`},
		{"missing_usage", `DELETE FROM usage`, http.StatusNotFound, `{}`},
	} {
		t.Run(tt.name, func(t *testing.T) {
			testutil.ResetTables(t, db)
			testutil.InsertUser(t, db, testutil.UserFixture{UserID: 1, Email: "file-app@ente.io", CreationTime: 1})
			...
			router := gin.New()
			router.POST("/admin/user/init-file-app", h.InitializeFileApp)
			request := httptest.NewRequest(http.MethodPost, "/admin/user/init-file-app", strings.NewReader(`{"userID":1}`))
