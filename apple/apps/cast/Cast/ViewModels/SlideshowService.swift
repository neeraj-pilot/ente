...
class RealSlideshowService: ObservableObject {
    ...
    private func fetchAllFiles(castPayload: CastPayload) async throws {
        ...
        while hasMore {
            if verboseFileLogging {
                print("Fetching files since time: \(sinceTime)")
            }
            ...
            await MainActor.run {
                if result.latestUpdateTime > self.lastUpdateTime {
                    print(
                        "Initial fetch updating lastUpdateTime: \(self.lastUpdateTime) → \(result.latestUpdateTime)"
                    )
        ...
        guard storedCastPayload == castPayload else { return }
        await print(
            "Initial diff fetch complete - total files cached: \(MainActor.run { allFiles.count })"
        )
        startPeriodicDiffPolling()
    }

    private func fetchFilesBatch(castPayload: CastPayload, sinceTime: Int64) async throws -> (
        files: [[String: Any]], hasMore: Bool, latestUpdateTime: Int64
    ) {
        guard storedCastPayload == castPayload else { throw CancellationError() }
        let url = baseURL.appendingPathComponent("cast/diff")
            .appending(queryItems: [URLQueryItem(name: "sinceTime", value: String(sinceTime))])
        ...
        guard httpResponse.statusCode == 200 else {
            if httpResponse.statusCode == 401 {
                ...
                throw CastError.serverError(
                    401, "Authentication expired - resetting to pairing mode"
                )
            }
            throw CastError.serverError(
    ...
    private func processDiffBatch(_ items: [[String: Any]], castPayload: CastPayload) async {
        ...
        let originalCurrentFile =
            currentFileIndex < allFiles.count ? allFiles[currentFileIndex] : nil
            ...
            if isDeleted {
                ...
                do {
                    if let file = try await decryptFileMetadata(
                        item: item, collectionKey: castPayload.collectionKey
                    ) {
                        guard storedCastPayload == castPayload else { return }
                        // Cast images and Live Photos, not standalone videos.
                        if file.isVideo, !file.isLivePhoto {
                            if verboseFileLogging {
                                print("Skipping video file: \(file.title) (ID: \(id))")
                            }
                            continue
                        }
    ...
    private func displaySlideAtCurrentIndex() async {
        ...
        do {
            ...
            let file = allFiles[currentFileIndex]
            print(
                "Failed to load file \(file.id): \(file.title) at index \(currentFileIndex) - \(error)"
            )
        ...
        if file.isLivePhoto {
            ...
            do {
                ...
                if let existing = videoTempFiles[file.id] {
                    ...
                    let fileExtension =
                        file.title.components(separatedBy: ".").last?.lowercased() ?? "mp4"
    ...
    private func startSlideTimer(withDuration customDuration: TimeInterval? = nil) {
        ...
        slideTimer = Timer.scheduledTimer(withTimeInterval: duration, repeats: false) {
            [weak self] _ in
            Task { @MainActor in
    ...
    private func skipToNextSlide() async {
        ...
        while retryCount < maxRetries {
            do {
                ...
                print(
                    "Attempting to load file \(file.id): \(file.title) at index \(currentFileIndex)"
                )
            ...
            for i in 1 ... prefetchCount {
                ...
                do {
                    ...
                    if prefetchCache.count > 5 {
                        let oldKeys = Array(
                            prefetchCache.keys.sorted().prefix(prefetchCache.count - 5)
                        )
                        for key in oldKeys {
    ...
    private func startPeriodicDiffPolling() {
        ...
        diffPollingTimer = Timer.scheduledTimer(
            withTimeInterval: diffPollingInterval, repeats: true
        ) { [weak self] _ in
            Task {
    ...
    private func downloadEncryptedFile(castPayload: CastPayload, fileID: Int) async throws -> Data {
        guard storedCastPayload == castPayload else { throw CancellationError() }
        let isProduction = baseURL == APIEndpoint.production
        let url =
            isProduction
        ...
        return try await download(
            URLRequest(url: fileURL), castPayload: castPayload, fileID: fileID
        )
    }

    private func download(_ request: URLRequest, castPayload: CastPayload, fileID: Int) async throws
        -> Data
    {
        let (data, response) = try await URLSession.shared.data(for: request)
        guard storedCastPayload == castPayload else { throw CancellationError() }
        ...
        if verboseFileLogging {
            print("Successfully downloaded \(data.count) bytes for file \(fileID)")
        }
        return data
    }
    ...
    private func downloadAndDecryptFileContent(castPayload: CastPayload, file: CastFile)
        async throws -> Data
    {
        ...
        if let cachedData = await getCachedFileContent(fileID: file.id) {
            if verboseFileLogging {
                print(
                    "Using cached content for file \(file.id): \(file.title) (\(cachedData.count) bytes)"
                )
            }
            return cachedData
        }
        ...
        if verboseFileLogging {
            print("Downloading and decrypting file \(file.id): \(file.title)")
        }

        let encryptedData = try await downloadEncryptedFile(
            castPayload: castPayload, fileID: file.id
        )
        if verboseFileLogging {
            print("Downloaded \(encryptedData.count) bytes")
        }
    ...
    private func decryptFileMetadata(item: [String: Any], collectionKey: String) async throws
        -> CastFile?
    {
    ...
    private func decryptFileKey(encryptedKey: String, nonce: String, collectionKey: String) throws
        -> Data
    {
        ...
        do {
            return try openSecretBox(
                ciphertext: encryptedKeyData, nonce: nonceData, key: collectionKeyData
            )
        } catch {
    ...
    private func decryptMetadata(encryptedData: String, decryptionHeader: String, fileKey: Data)
        throws -> Data
    {
        ...
        print(
            "XChaCha20: encrypted=\(encryptedBytes.count)b, header=\(headerBytes.count)b, key=\(fileKey.count)b"
        )

        do {
            let decryptedData = try decryptSecretStream(
                encryptedData: encryptedBytes, header: headerBytes, key: fileKey
            )
            print("Metadata decrypted using Rust crypto: \(decryptedData.count) bytes")
            return decryptedData
        } catch {
            throw CastError.decryptionError(
                "XChaCha20-Poly1305 decryption failed for metadata: \(error)"
            )
        }
    }

    private func decryptFileContent(encryptedData: Data, fileKey: Data, decryptionHeader: String)
        throws -> Data
    {
        ...
        if verboseDecryptionLogging {
            print(
                "File decryption: encrypted=\(encryptedData.count)b, header=\(headerBytes.count)b, key=\(fileKey.count)b"
            )
        }

        do {
            return try decryptSecretStream(
                encryptedData: encryptedData, header: headerBytes, key: fileKey
            )
        } catch {
    ...
    private func cleanupExpiredCache(validFileIDs: Set<Int>, castPayload: CastPayload) async {
        ...
        if removedCount > 0 {
            ...
            print(
                "Cache cleanup complete - removed \(removedCount) expired files, now \(newStats.count) files (\(newStats.totalSize) bytes)"
            )
        }
    }
...
func extractLivePhotoComponents(from zipData: Data) throws -> LivePhotoComponents {
    ...
    let extractDirectory = tempDirectory.appendingPathComponent(
        "livephoto_extract_\(UUID().uuidString)"
    )
    ...
    do {
        ...
        let enumerator = FileManager.default.enumerator(
            at: extractDirectory, includingPropertiesForKeys: resourceKeys
        )
        ...
        if imageData == nil || videoData == nil {
            let contents = try FileManager.default.contentsOfDirectory(
                at: extractDirectory, includingPropertiesForKeys: nil
            )
            if imageData == nil {
            ...
            if videoData == nil {
                if let guessVideo = contents.first(where: {
                    ["bin", "dat"].contains($0.pathExtension.lowercased())
                }) {
        ...
        if imageData == nil, videoData == nil {
            throw CastError.decryptionError(
                "No valid image or video components found in live photo zip"
            )
        }
