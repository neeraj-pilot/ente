...
struct SlideshowView: View {
    ...
    init(
        imageData: Data? = nil, videoData: Data? = nil, isVideo: Bool = false,
    ...
    private func decodedUIImage(from data: Data) -> UIImage? {
        ...
        if let uiImage = UIImage(data: data) {
            ...
            if let cg = uiImage.cgImage {
                return UIImage(
                    cgImage: cg, scale: uiImage.scale, orientation: uiImage.imageOrientation)
            }
            return uiImage
        } else {
        ...
        VStack {
            ...
            HStack {
                ...
                VStack(spacing: 20) {
                    ...
                    HStack(spacing: 40) {
                        NavigationHint(
                            icon: "chevron.left.circle.fill", label: "Previous", direction: .leading
                        )
                        NavigationHint(
                            icon: isPaused ? "play.circle.fill" : "pause.circle.fill",
                            label: isPaused ? "Resume" : "Pause", direction: .center)
                        NavigationHint(
                            icon: "chevron.right.circle.fill", label: "Next", direction: .trailing)
                    }
