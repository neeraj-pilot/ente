...
class RustWallClient @Inject constructor(
    ...
    override suspend fun listOwnedWalls(): List<Wall> =
        ...
        }.map { it.model.also(::rememberWallProfile) }
    ...
    override suspend fun wall(bySlug: String): Wall? {
        ...
        return json.decodeFromString<WireWall>(payload).model.also(::rememberWallProfile)
    }
    ...
    override suspend fun createWall(slug: String, displayName: String?, bio: String?): Wall =
        ...
        }.model.also(::rememberWallProfile)
    ...
    private suspend fun remember(page: WirePage<WirePost>): Page<Post> {
        return Page(
    ...
    private suspend fun remember(post: WirePost): Post {
        ...
        return post.model.hydratedAuthor()
    }

    private suspend fun rememberComments(page: WirePage<WireComment>): Page<Comment> {
        return Page(
            items = page.items.map { remember(it) },
            nextCursor = page.nextCursor,
        )
    ...
    private suspend fun remember(comment: WireComment): Comment {
        commentPosts[comment.id] = comment.postId
        return comment.model.hydratedAuthor()
    }

    private suspend fun remember(request: WireFollowRequest): FollowRequest {
        return request.model.hydratedFrom()
    }

    private fun rememberWallProfile(wall: Wall) {
        val displayName = wall.displayName.nonBlank() ?: return
        displayNames[wall.slug.cacheKey()] = displayName
    }

    private suspend fun Post.hydratedAuthor(): Post {
        val resolved = authorDisplayName.nonBlank() ?: displayNameForSlug(authorSlug)
        return if (resolved == authorDisplayName) this else copy(authorDisplayName = resolved)
    }

    private suspend fun Comment.hydratedAuthor(): Comment {
        val resolved = authorDisplayName.nonBlank() ?: displayNameForSlug(authorSlug)
        return if (resolved == authorDisplayName) this else copy(authorDisplayName = resolved)
    }

    private suspend fun FollowRequest.hydratedFrom(): FollowRequest {
        val resolved = fromDisplayName.nonBlank() ?: displayNameForSlug(fromSlug)
        return if (resolved == fromDisplayName) this else copy(fromDisplayName = resolved)
    }

    private suspend fun displayNameForSlug(slug: String): String? {
        displayNames[slug.cacheKey()]?.let { return it }
        val displayName = runCatching { wall(slug)?.displayName.nonBlank() }.getOrNull()
        if (displayName != null) {
            displayNames[slug.cacheKey()] = displayName
        }
        return displayName
    }
    ...
    private data class WireFollowRequest(
        ...
        val model: FollowRequest
            get() = FollowRequest(
                ...
                wallId = wallId,
                createdAt = parseInstantOrFallback(createdAt),
