...
class FileUploader {
  ...
  final _existingUploadResolver = ExistingUploadResolver();
  ...
  Future<EnteFile> _tryToUpload(
    ...
    try {
      ...
      if (isUpdatedFile) {
        ...
        final resolution = ownerID == null
            ? const UploadRequired()
            : await _existingUploadResolver.resolve(
                hashData: mediaUploadData.hashData,
                fileToUpload: file,
                destinationCollectionID: collectionID,
                ownerID: ownerID,
              );
        switch (resolution) {
          case ResolvedExistingUpload(file: final resolvedFile):
            debugPrint(
