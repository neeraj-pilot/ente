...
#[cfg(test)]
mod tests {
    use ente_location::{CountryCode, CountryIndex};
    use geo::{LineString, MultiPolygon, Polygon};

    use super::{Area, encode};

    #[test]
    fn builds_country_geometry_with_a_hole() {
        let polygon = Polygon::new(
            ring(-10.0, -10.0, 10.0, 10.0),
            vec![ring(-1.0, -1.0, 1.0, 1.0)],
        );
        let bytes = encode(&[Area {
            code: *b"AA",
